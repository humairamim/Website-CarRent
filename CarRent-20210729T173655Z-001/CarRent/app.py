...
def dashboard():
    ...
    if current_user.UserType == 'tenant':
    ...
    elif current_user.UserType == 'owner':
        products = session.query(Product).filter_by(Owner=current_user.ID).all()
        ...
        for product in products:
            single_product = {}
            print(product.Owner)
            owner = session.query(Users).filter_by(ID=product.Owner).first()
            owner_name = owner.FullName
            print(owner.FullName)
            single_product = {'owner' : owner_name,'product_id' : product.ID,'product_name' : product.ProductName, 'product_description': product.ProductDescription, 'product_price' : product.Price }
...
# Add rent only allowed for owner
...
def add_product():
    if current_user.UserType == 'owner':
        ...
        if product_form.validate_on_submit():
            newProduct = Product(ProductName=product_form.ProductName.data, ProductDescription=product_form.ProductDescription.data, Category=product_form.Category.data, Price=product_form.Price.data, Owner=current_user.ID)
...
def CarItems():
    ...
    return render_template('CarItems.html', categories=categories, all_products=all_products)
